...
//     menu.forEach((item, index) => {
//       formData.append(`menu[${index}][name]`, item.name);
//       formData.append(`menu[${index}][imageUrl]`, item.imageUrl);
//     });
...
//             value={item.imageUrl}
//             onChangeText={(value) => handleMenuItemChange(index, "imageUrl", value)}
//           />
//         </View>
//       ))}
//       <TouchableOpacity style={styles.button} onPress={handleAddMenuItem}>
...
const AddRestaurantScreen = ({ navigation }) => {
  ...
  const [dressCode, setDressCode] = useState("");
  const [amenities, setAmenities] = useState([]);
  const [photos, setPhotos] = useState([]); // Updated to photos
  const [menu, setMenu] = useState([]);
  const [location, setLocation] = useState({ latitude: "", longitude: "" });
  const [availableTimeSlots, setAvailableTimeSlots] = useState([]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const restaurantData = {
        ...
        availableTimeSlots,
        photos: photos.filter(url => url.trim() !== ''), // Remove empty strings
        menu: menu.map(item => ({
          name: item.name,
          image: item.imageUrl // Note: changed from imageUrl to image to match schema
        }))
      };
